<!DOCTYPE html>
<html lang="es">
<head>
	<meta charset="UTF-8">
	<meta http-equiv="X-UA-Compatible" content="IE=edge">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<link rel="stylesheet" href="./css/desflujo.css">
	<title>SEGUNDO EXAMEN</title>
</head>
<body>
	<?php
	include "conexion.inc.php";
	$flujo=$_GET["flujo"];
	$proceso=$_GET["proceso"];
	$sql="select * from flujoproceso where flujo='".$flujo."' and proceso='".$proceso."'";
	$resultado=mysqli_query($conn, $sql);
	$fila=mysqli_fetch_array($resultado);
	//print_r($fila);
	include $fila['formulario'].'.cab.inc.php';

	?>
	<form action="motflujo.php" method="GET" class="formulario1">

		<?php include $fila['formulario'].'.inc.php';?>
		<br>
		<input type="hidden" value="<?php echo $fila['formulario'];?>" name="formulario"/>
		<input type="hidden" value="<?php echo $flujo?>" name="flujo"/>
		<input type="hidden" value="<?php echo $proceso?>" name="proceso"/>
		<input class="flujo" type="submit" value="Anterior" name="Anterior"/>
		<input class="flujo" type="submit" value="Siguiente" name="Siguiente"/>
	</form>
</body>
</html>